jQuery(document).ready(function ($) {
				...
				if (response.success) {
					...
					$.each(response.files, function (file, data) {
						...
						if ('index.html' != file && 'notification.html' != file) {
							html += '<a href="" class="thickbox-remove-file mymail-icon" data-slug="' + response.slug + '" data-file="' + file + '"></a>';
						}
	...
	$(document)
		...
		.on('click', '.thickbox-remove-file', function () {
			...
			if (confirm(sprintf(mymailL10n.delete_template_file, file, slug))) {
