...
class MyMailSubscribers {
	...
	public function output_referer( $id ) {
		...
		if ( $meta->referer ):
			...
			switch ( $meta->referer ) {
			case 'import': ?>
            <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <span><?php printf( __( 'import on %s', 'mymail' ), date( $timeformat, $subscriber->added + $timeoffset ) ) ?></span>
                    <?php break;
			case 'wpuser':
			case '/wp-admin/user-new.php':
			case '/wp-login.php?action=register': ?>
            <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <span><?php printf( __( 'WP user on %s', 'mymail' ), date( $timeformat, $subscriber->added + $timeoffset ) ) ?></span>
                    <?php break;
			case 'backend': ?>
            <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <span><?php printf( __( 'Backend on %s', 'mymail' ), date( $timeformat, $subscriber->added + $timeoffset ) ) ?></span>
                    <?php break;
			case 'extern': ?>
            <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <span><?php printf( __( 'an extern form on %s', 'mymail' ), date( $timeformat, $subscriber->added + $timeoffset ) ) ?></span>
                    <?php break;
			case '/': ?>
            <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <span><?php printf( __( 'Homepage on %s', 'mymail' ), date( $timeformat, $subscriber->added + $timeoffset ) ) ?></span>
                    <?php break;
			default:
				if ( preg_match( '#^wpcomment_(\d+)#', $meta->referer, $match ) ):
					$comment = get_comment( $match[1] );?>

                <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <span><?php printf( __( '%1$s on %2$s', 'mymail' ), '<a href="' . get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment->comment_ID . '">' . __( 'Comment', 'mymail' ) . '</a>', date( $timeformat, $subscriber->added + $timeoffset ) ) ?></span>
                <?php elseif ( preg_match( '#^https?://#', $meta->referer, $match ) ): ?>

            <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <a href="<?php echo $meta->referer ?>"><?php echo $meta->referer ?></a>

            <?php else: ?>

        <strong><?php esc_html_e( 'via', 'mymail' );?></strong> <?php echo $meta->referer ?>
...
?>
            <br><strong><?php esc_html_e( 'Form', 'mymail' )?> #<?php echo $form->ID ?>:</strong> <a href="<?php echo admin_url( 'edit.php?post_type=newsletter&page=mymail_forms&ID=' . $form->ID ) ?>"><?php echo $form->name ?></a>
            <?php endif;
	...
	public function add_from_wp_user( $user_id, $userdata = array() ) {
		...
		if ( empty( $user ) ) {
			return new WP_Error( 'no_user', __( 'User doesn\'t exist!', 'mymail' ) );
		}
		...
		if ( $subscriber_exists ) {
			return new WP_Error( 'subscriber_exists', __( 'Subscriber already exists', 'mymail' ) );
		}
	...
	public function remove( $subscriber_ids, $status = null, $remove_actions = true ) {
		...
		$subscriber_ids = !is_array( $subscriber_ids ) ? array( intval( $subscriber_ids ) ) : array_filter( $subscriber_ids, 'is_numeric' );
		$statuses = !is_null( $status ) ? ( !is_array( $status ) ? array( intval( $status ) ) : array_filter( $status, 'is_numeric' ) ) : null;

		if ( empty( $subscriber_ids ) ) return true;
		...
		if ( $delete_wp_user = ( mymail_option( 'delete_wp_user' ) && current_user_can( 'delete_users' ) ) ) {
			...
			$sql = "SELECT wp_id FROM {$wpdb->prefix}mymail_subscribers AS a WHERE a.wp_id != 0 AND a.wp_id != %d AND a.ID IN (" . implode( ',', $subscriber_ids ) . ")";
