...
class MyMailSettings {
			...
			if ( isset( $_GET['mymail_create_homepage'] ) && $_GET['mymail_create_homepage'] ) {
				...
				if ( $homepage = mymail_option( 'homepage' ) ) {
					...
					if ( $id = wp_insert_post( $mymail_homepage ) ) {
						...
						wp_redirect( 'post.php?post=' . $id . '&action=edit&message=10&remove_notice=mymail_no-homepage' );
						exit;
					}
	...
	public function maybe_repair_settings() {
		...
		global $wpdb, $mymail_options;

		if ( $mymail_options ) {
			return;
		}

		$serialized_string = $wpdb->get_var( "SELECT option_value FROM {$wpdb->options} WHERE option_name = 'mymail_options'" );
		if ( !$serialized_string ) {
			return;
		}

		$mymail_options = mymail( 'helper' )->unserialize( $serialized_string );
		if ( update_option( 'mymail_options', $mymail_options ) ) {
			mymail_notice( '<strong>' . sprintf( __( 'There was a problem in your MyMail settings which has been automatically fixed! Either way it\'s good to check %s if everything is in place.', 'mymail' ), '<a href="options-general.php?page=newsletter-settings&mymail_remove_notice=error_settings">' . __( 'the settings page', 'mymail' ) . '</a>' ) . '</strong>', 'error', false, 'error_settings' );
		}
	...
	public function newsletter_settings() {
		...
		if ( !$mymail_options ): ?>
			...
			<h2><?php esc_html_e( 'Ooops, seems your settings are missing or broken :(', 'mymail' ); ?></h2>

			<p><a href="options-general.php?page=newsletter-settings&reset-settings=1&_wpnonce=<?php echo wp_create_nonce( 'mymail-reset-settings' ) ?>" class="button button-primary button-large"><?php esc_html_e( 'Reset all settings now', 'mymail' ); ?></a></p>
			</div>
	...
	public function verify( $options ) {
		...
		foreach ( $options as $id => $value ) {
			...
			//skip certain values
			if ( in_array( $id, array( 'dkim_private_hash' ) ) ) continue;
			...
			case 'purchasecode':
				...
				if ( $old != $value && $value ) {
					...
					if ( !preg_match( '#^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$#', $value ) ) {
						add_settings_error( 'mymail_options', 'mymail_options', sprintf( __( 'The provided Purchase Code %s is invalid!', 'mymail' ), '"' . $value . '"' ) );
						$value = '';
					} else {
						$verified = UpdateCenterPlugin::verify( MYMAIL_SLUG, $value );
						if ( is_wp_error( $verified ) ) {

							$code = $verified->get_error_code();

							if ( 681 != $code ) {
								$message = mymail()->get_update_error( $code );
								add_settings_error( 'mymail_options', 'mymail_options', $message );
								$value = '';
							}

						} else {
							add_settings_error( 'mymail_options', 'mymail_options', __( 'Thanks, your Purchase Code is valid!', 'mymail' ), 'updated' );
						}
	...
	public function get_system_info( $space = 30 ) {
		...
		$wp_id = mymail( 'subscribers' )->wp_id() === false ? 'ERROR: ' . $wpdb->last_error : 'OK';
