...
class MyMailManage {
	...
	public function scripts_styles() {
		...
		wp_localize_script( 'mymail-manage-script', 'mymailL10n', array(
				...
				'error_export' => __( 'There was an error while exporting', 'mymail' ),
	...
	public function ajax_import_subscribers_upload_handler() {
		...
		$wpdb->query( "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}mymail_temp_import (ID bigint(20) NOT NULL AUTO_INCREMENT, data longtext NOT NULL, identifier char(13) NOT NULL, PRIMARY KEY (ID) ) $collate" );
...
(SELECT data FROM {$wpdb->prefix}mymail_temp_import WHERE identifier = %s ORDER BY ID ASC LIMIT 1) AS first, (SELECT data FROM {$wpdb->prefix}mymail_temp_import WHERE identifier = %s ORDER BY ID DESC LIMIT 1) AS last", $identifier, $identifier ) );
		...
		$meta_other = array(
			'_lists' => __( 'Lists', 'mymail' ) .' ('.__( 'comma separated', 'mymail' ).')',
...
';
				...
				foreach ( $list as $line ) {
					...
					if ( is_wp_error( $subscriber_id ) ) {
						...
						if ( !empty( $list_ids ) ) {
							...
							mymail( 'subscribers' )->assign_lists( $subscriber_id, $list_ids, $bulkdata['existing'] == 'overwrite' );
						}
	...
	public function ajax_export_contacts() {
		...
		if ( !current_user_can( 'mymail_export_subscribers' ) ) {
			$return['msg'] = __( 'You are not allowed to export subscribers!', 'mymail' );
		...
		if ( $count ) {
			...
			update_option( 'mymail_export_filename', $filename );
	...
	public function ajax_do_export() {
		...
		$filename = get_option( 'mymail_export_filename' );

		if ( !file_exists( $filename ) || !is_writeable( $filename ) ) {
			$return['msg'] = __( 'Not able to write export file', 'mymail' );
	...
	public function ajax_delete_contacts() {
		...
		if ( isset( $d['nolists'] ) ) {
			...
			$subscriber_ids = mymail( 'campaigns' )->get_subscribers_by_lists( null, null, $statuses, true );

			$return['success'] = mymail( 'subscribers' )->remove( $subscriber_ids, $statuses, isset( $d['remove_actions'] ) );
		...
		if ( !empty( $listids ) ) {
			...
			$subscriber_ids = mymail( 'campaigns' )->get_subscribers_by_lists( $listids, null, $statuses, true );

			$return['success'] = mymail( 'subscribers' )->remove( $subscriber_ids, $statuses, isset( $d['remove_actions'] ) );

			if ( $return['success'] && !empty( $listids ) ) {

				mymail( 'lists' )->remove( $listids );
...
?>
    ...
    <div id="drag-drop-area">
        ...
        <p class="drag-drop-info"><?php esc_html_e( 'Drop your list here', 'mymail' );?></p>
    ...
    <div id="html-upload-ui" class="hide-if-js">
        <p id="async-upload-wrap">
            <label class="screen-reader-text" for="async-upload"><?php esc_html_e( 'Upload', 'mymail' );?></label>
    ...
    <p class="max-upload-size"><?php printf( __( 'Maximum upload file size: %s.', 'mymail' ), esc_html( $upload_size_unit . $sizes[$u] ) );?> <?php esc_html_e( 'Split your lists into max 50.000 subscribers each.', 'mymail' );?></p>
    <p class="max-upload-size"><?php esc_html_e( 'Try pasting your data into the textarea on the right if you have troubles with special characters.', 'mymail' );?></p>
    <?php
		if ( ( $is_IE || $is_opera ) && $max_upload_size > 100 * 1024 * 1024 ) {?>
        <span class="big-file-warning"><?php esc_html_e( 'Your browser has some limitations uploading large files with the multi-file uploader. Please use the browser uploader for files over 100MB.', 'mymail' );?></span>
    <?php }
