...
<ul class="category-tabs">
	...
	?>"><a href="#regular-campaign"><?php esc_html_e( 'Regular Campaign', 'mymail' );?></a></li>
	<li class="<?php if ( $is_autoresponder ) {
	echo 'tabs';
}
?>"><a href="#autoresponder"><?php esc_html_e( 'Auto Responder', 'mymail' );?></a></li>
</ul>
...
printf( __( 'Server time: %s %s', 'mymail' ), '<span title="' . date( $timeformat, $now + $timeoffset ) . '">' . date( 'Y-m-d', $now + $timeoffset ) . '</span>', '<span class="time" data-timestamp="' . ( $now + $timeoffset ) . '">' . date( 'H:i:s', $now + $timeoffset ) . '</span>' );

elseif ( $post->post_status == 'finished' ):

	printf( __( 'This campaign has been sent on %s. You cannot edit it anymore', 'mymail' ), '<strong>' . date( $timeformat, $this->post_data['finished'] + $timeoffset ) . '</strong>' );
...
<?php if ( $editable ): ?>
<label>
	<input name="mymail_data[active]" id="mymail_data_active" value="1" type="checkbox" <?php echo ( $this->post_data['active'] && !$is_autoresponder ) ? 'checked' : '' ?> <?php echo ( !$editable ) ? ' disabled' : '' ?>> <?php esc_html_e( 'send this campaign', 'mymail' )?>
</label>
...
?>">
	...
	<?php printf( _x( 'on %1$s @ %2$s', 'send campaign "on" (date) "at" (time)', 'mymail' ),
...
?>
	<br><label title="<?php esc_html_e( 'Send this campaign based on the subscribers timezone if known', 'mymail' )?>"><input type="checkbox" class="timezone" name="mymail_data[timezone]" value="1" <?php checked( $this->post_data['timezone'] );?>> <?php esc_html_e( 'Use Subscribers timezone', 'mymail' )?></label>
	<?php endif;?>
</div>
...
?>
		<p>
			<div class="progress paused"><span class="bar" style="width:<?php echo $p ?>%"></span><span>&nbsp;<?php printf( __( '%1$s of %2$s sent', 'mymail' ), number_format_i18n( $sent ), number_format_i18n( $totals ) ) ?></span><var><?php echo $p ?>%</var></div>
		</p>
	<?php endif;?>
...
?>
	<label>
		<input name="mymail_data[active_autoresponder]" id="mymail_data_autoresponder_active" value="1" type="checkbox" <?php checked( ( $this->post_data['active'] && $is_autoresponder ), true )?> <?php echo ( !$editable ) ? ' disabled' : '' ?>> <?php esc_html_e( 'send this auto responder', 'mymail' )?>
	</label>
...
?>
				...
				<span class="autoresponder_after"><?php esc_html_e( 'after', 'mymail' );?></span>
				<span class="autoresponder_before"><?php esc_html_e( 'before', 'mymail' );?></span>
				<select class="autoresponder_before_after" name="mymail_data[autoresponder][before_after]">
					<option value="1" <?php selected( $autoresponderdata['before_after'], 1 );?>><?php esc_html_e( 'after', 'mymail' );?></option>
					<option value="-1" <?php selected( $autoresponderdata['before_after'], -1 );?>><?php esc_html_e( 'before', 'mymail' );?></option>
				</select>
		</p>
		<p>
...
?>
		...
		<div class="mymail_autoresponder_more autoresponderfield-mymail_subscriber_insert autoresponderfield-mymail_subscriber_unsubscribed">
			...
			<?php esc_html_e( 'only for subscribers who signed up', 'mymail' );?>
			</span>
			<span class="mymail_autoresponder_more autoresponderfield-mymail_subscriber_unsubscribed">
			<?php esc_html_e( 'only for subscribers who canceled their subscription', 'mymail' );?>
			</span>
			<?php
esc_html_e( 'after', 'mymail' );
$timestamp = $this->post_data['timestamp'] ? $this->post_data['timestamp'] : $now;

printf( _x( '%1$s @ %2$s', 'send campaign "on" (date) "at" (time)', 'mymail' ),
...
?>
		...
		<div class="mymail_autoresponder_more autoresponderfield-mymail_subscriber_unsubscribed">
			<p class="description">
			<?php esc_html_e( 'Keep in mind it is bad practice to send campaigns after subscribers opt-out so use this option for "Thank you" messages or surveys.', 'mymail' );?>
			</p>
		</div>
...
$type .= '</select>';
printf( __( 'create a new campaign every time a new %s has been published', 'mymail' ), $type );
?>
			</p>
			<p>
			<?php
if ( mymail_option( 'trackcities' ) ):
?>
				<label title="<?php esc_html_e( 'Send this campaign based on the subscribers timezone if known', 'mymail' )?>"><input type="checkbox" class="autoresponder-timezone" name="mymail_data[autoresponder][post_published_timezone]" value="1" <?php checked( $this->post_data['timezone'] );?>> <?php esc_html_e( 'Use Subscribers timezone', 'mymail' )?></label>
				<?php endif;?>
			</p>
...
if ( $taxes ) {
	printf( __( 'only if in %s', 'mymail' ), $taxes );
}
?>
				</div>
				<p>
				<?php
printf( _n( 'always skip %s release', 'always skip %s releases', $autoresponderdata['post_count'], 'mymail' ), $count );
?>
				</p>
			</div>
		</div>
...
$time_frame .= '</select>';
printf( _x( 'create a new campaign every %1$s%2$s', 'every [x] [timeframe] starting [startdate]', 'mymail' ), $interval, $time_frame );
?>
			</p>
			<?php
echo '<h4>' . __( 'next schedule', 'mymail' ) . '</h4>';
?>
			<p><?php printf( _x( 'on %1$s @ %2$s', 'send campaign "on" (date) "at" (time)', 'mymail' ),
...
if ( mymail_option( 'trackcities' ) ):
?>
		<label title="<?php esc_html_e( 'Send this campaign based on the subscribers timezone if known', 'mymail' )?>"><input type="checkbox" class="autoresponder-timezone" name="mymail_data[autoresponder][timebased_timezone]" value="1" <?php checked( $this->post_data['timezone'] );?>> <?php esc_html_e( 'Use Subscribers timezone', 'mymail' )?></label>
		<?php endif;?>
			</p>
			<p>
				<label><input type="checkbox" name="mymail_data[autoresponder][endschedule]" class="mymail_autoresponder_timebased-end-schedule" <?php checked( $autoresponderdata['endschedule'] );?> value="1"> <?php esc_html_e( 'end schedule', 'mymail' );?></label>
...
printf( _x( 'on %1$s @ %2$s', 'send campaign "on" (date) "at" (time)', 'mymail' ),
...
?>
				<span class="description"><?php esc_html_e( 'set an end date for your campaign', 'mymail' );?></span>
				</div>
...
for ( $i = $start_at; $i < 7 + $start_at; $i++ ) {
	...
	echo '<label title="' . $wp_locale->weekday[$j] . '" class="weekday"><input name="mymail_data[autoresponder][weekdays][]" type="checkbox" value="' . $j . '" ' . checked( ( in_array( $j, $autoresponderdata['weekdays'] ) || !$autoresponderdata['weekdays'] ), true, false ) . '>' . $wp_locale->weekday_initial[$wp_locale->weekday[$j]] . '&nbsp;</label> ';
}
?>
			</p>

			<label><input type="checkbox" name="mymail_data[autoresponder][time_conditions]" id="time_extra" value="1" <?php checked( isset( $autoresponderdata['time_conditions'] ) )?>> <?php esc_html_e( 'only if', 'mymail' );?></label>
...
$type .= '</select><br>';
printf( __( '%1$s %2$s have been published', 'mymail' ), $count, $type );
?>
				</p>
			</div>
		</div>
...
printf( __( 'Next issue: %s', 'mymail' ), $issue );
?>
				</p>
				<p class="description">
				<?php
printf( __( 'Use the %s tag to display the current issue in the campaign', 'mymail' ), '<code>{issue}</code>' );
?>
				</p>
		</div>
...
printf( _n( '%1$s matching %2$s has been published', '%1$s matching %2$s have been published', $autoresponderdata['post_count_status'], 'mymail' ), '<strong>' . $autoresponderdata['post_count_status'] . '</strong>', '<strong><a href="edit.php?post_type=' . $post_type . '">' . _n( $pts[$post_type]->labels->singular_name, $pts[$post_type]->labels->name, $autoresponderdata['post_count_status'] ) . '</a></strong>' );
?>
				<br><label><input type="checkbox" name="post_count_status_reset" value="1"> <?php esc_html_e( 'reset counter', 'mymail' );?></label>
				</p>
...
?>
	...
	?>><?php printf( __( 'every %1$s %2$s', 'mymail' ), $amount, $unit ) ?></span></label><br>
	...
	?>><?php esc_html_e( 'on the exact date', 'mymail' );?></span></label>
			</p>
			<p>
				<?php
	printf( __( 'of the users %1$s value', 'mymail' ), $uservalue );
} else {

	esc_html_e( 'No custom date fields found!', 'mymail' );
...
if ( mymail_option( 'trackcities' ) ):
?>
				<label title="<?php esc_html_e( 'Send this campaign based on the subscribers timezone if known', 'mymail' )?>"><input type="checkbox" class="autoresponder-timezone" name="mymail_data[autoresponder][usertime_timezone]" value="1" <?php checked( $this->post_data['timezone'] );?>> <?php esc_html_e( 'Use Subscribers timezone', 'mymail' )?></label>
				<?php endif;?>

			</p>
			<p><label>
				<input type="checkbox" name="mymail_data[autoresponder][usertime_once]" value="1" <?php checked( $autoresponderdata['once'] )?>> <?php esc_html_e( 'send campaign only once', 'mymail' );?>
			</label></p>
		</div>
...
?>
				<p>
					<select name="mymail_data[autoresponder][followup_action]">
						<option value="1" <?php selected( $autoresponderdata['followup_action'], 1 );?>><?php esc_html_e( 'has been sent', 'mymail' );?></option>
						<option value="2" <?php selected( $autoresponderdata['followup_action'], 2 );?>><?php esc_html_e( 'has been opened', 'mymail' );?></option>
						<option value="3" <?php selected( $autoresponderdata['followup_action'], 3 );?>><?php esc_html_e( 'has been clicked', 'mymail' );?></option>
					</select>
				</p><fieldset>
					<label><?php esc_html_e( 'Campaign', 'mymail' );?>
...
?>
				...
				<?php else: ?>
			<p><?php esc_html_e( 'No campaigns available', 'mymail' );?></p>
			<?php endif;?>
		</div>
		<div class="mymail_autoresponder_more autoresponderfield-mymail_autoresponder_hook">
			<p><label>
			<?php esc_html_e( 'Hook used to trigger campaign', 'mymail' );?> (<abbr title="<?php esc_attr_e( 'use `do_action("hook_name")`, or `do_action("hook_name", $subscriber_id)` to trigger this campaign', 'mymail' );?>">?</abbr>)
			...
			<p><label>
			<?php esc_html_e( 'Priority', 'mymail' );?>:
				<select name="mymail_data[autoresponder][priority]">
					<option value="5" <?php selected( $autoresponderdata['priority'], 5 );?>><?php esc_html_e( 'High', 'mymail' );?></option>
					<option value="10" <?php selected( $autoresponderdata['priority'], 10 );?>><?php esc_html_e( 'Normal', 'mymail' );?></option>
					<option value="15" <?php selected( $autoresponderdata['priority'], 15 );?>><?php esc_html_e( 'Low', 'mymail' );?></option>
				</select>
			</label></p>
			<p><label>
				<input type="checkbox" name="mymail_data[autoresponder][hook_once]" value="1" <?php checked( $autoresponderdata['once'] )?>> <?php esc_html_e( 'send campaign only once', 'mymail' );?>
			</label></p>
		</div>
...
<p>
	...
	<button type="button" class="button mymail_spamscore" title="<?php esc_html_e( 'check your spam score', 'mymail' );?> (beta)">Spam Score</button>
	<span class="spinner" id="delivery-ajax-loading"></span>
	<input type="button" value="<?php esc_html_e( 'Send Test', 'mymail' )?>" class="button mymail_sendtest">
...
<?php elseif ( $post->post_status == 'active' ): ?>
	<p>
	<?php printf( __( 'This campaign has been started on %1$s, %2$s ago', 'mymail' ), '<br><strong>' . date( $timeformat, $this->post_data['timestamp'] + $timeoffset ), human_time_diff( $now, $this->post_data['timestamp'] ) . '</strong>' ); ?>
	</p>
<?php if ( $sent && !$is_autoresponder ):
...
$p = round( $this->get_sent_rate( $post->ID ) * 100 );
?>
		<div class="progress"><span class="bar" style="width:<?php echo $p ?>%"></span><span>&nbsp;<?php printf( __( '%1$s of %2$s sent', 'mymail' ), number_format_i18n( $sent ), number_format_i18n( $totals ) ) ?></span><var><?php echo $p ?>%</var></div>
...
printf( __( 'finished in approx. %s', 'mymail' ), '<strong>' . $timeleft . '</strong>' );
?>
		</p>
		<?php endif;?>
<?php endif;?>
<?php elseif ( $is_autoresponder ): ?>
	<p>
	<?php printf( __( 'You have to %s to change the delivery settings', 'mymail' ), '<a href="post.php?post=' . $post_id . '&action=edit">' . __( 'switch to the edit mode', 'mymail' ) . '</a>' ); ?>
	</p>
...
?>
				<p><?php printf( __( 'This campaign is based on an %s', 'mymail' ), '<a href="post.php?post=' . $this->post_data['parent_id'] . '&action=edit&showstats=1">' . __( 'auto responder campaign', 'mymail' ) . '</a>' ); ?>
				</p>
		<?php
	endif;
