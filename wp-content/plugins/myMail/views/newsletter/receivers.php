...
?>
		<ul class="category-tabs">
			<li class="tabs"><?php esc_html_e( 'send campaign to these lists', 'mymail' );?></li>
		</ul>
...
?>>
			<label><input type="checkbox" id="all_lists"> <?php esc_html_e( 'toggle all', 'mymail' );?></label>
				<div>
					<ul class="default">
						<?php
...
?>
			...
			<ul>
				<li><label><input id="ignore_lists" type="checkbox" name="mymail_data[ignore_lists]" value="1" <?php checked( $ignore_lists )?>> <?php esc_html_e( 'List doesn\'t matter', 'mymail' );?> </label></li>
			</ul>
			<label><input type="checkbox" name="mymail_data[list_conditions]" id="list_extra" value="1" <?php checked( isset( $listdata['conditions'] ) )?>> <?php esc_html_e( 'only if', 'mymail' );?></label>
			<div id="mymail_list_advanced" <?php if ( !isset( $listdata['conditions'] ) ) {echo 'style="display:none"';}?>>
				...
				<select id="mymail_list_operator" class="widefat" name="mymail_data[list][operator]">
					<option value="OR"<?php selected( $listdata['operator'], 'OR' )?> title="<?php esc_html_e( 'or', 'mymail' );?>"><?php esc_html_e( 'one of the conditions is true', 'mymail' );?></option>
					<option value="AND"<?php selected( $listdata['operator'], 'AND' )?> title="<?php esc_html_e( 'and', 'mymail' );?>"><?php esc_html_e( 'all of the conditions are true', 'mymail' );?></option>
				</select>
				</p>
...
?>
				<div class="mymail_list_condition" id="mymail_list_condition_<?php echo $i; ?>">
					<div class="mymail-list-operator">
						<span class="operator-and"><?php esc_html_e( 'and', 'mymail' );?></span>
						<span class="operator-or"><?php esc_html_e( 'or', 'mymail' );?></span>
					</div>
					<div><a class="remove-condition" title="<?php esc_html_e( 'remove condition', 'mymail' );?>">&#10005;</a></div>
		...
		echo '<option value="' . $value . '"' . selected( $condition['field'], $value, false ) . '>' . $name . '</option>';
					...
					<optgroup label="<?php esc_html_e( 'Custom Fields', 'mymail' );?>">
		...
		echo '<option value="' . $value . '"' . selected( $condition['field'], $value, false ) . '>' . $customfield['name'] . '</option>';
					...
					<optgroup label="<?php esc_html_e( 'Meta Data', 'mymail' );?>">
					<?php foreach ( $meta as $value => $name ) {
		echo '<option value="' . $value . '"' . selected( $condition['field'], $value, false ) . '>' . $name . '</option>';
	}?>
					</optgroup>
					<optgroup label="<?php esc_html_e( 'WordPress User Meta', 'mymail' );?>">
...
?>
				 ...
				 <a class="add-condition" title="<?php esc_html_e( 'add condition', 'mymail' );?>"><?php esc_html_e( 'add condition', 'mymail' );?></a>
		 	</div>
		</div>
			<p class="totals"><?php esc_html_e( 'Total receivers', 'mymail' );?>: <span id="mymail_total"><?php echo number_format_i18n( $total ) ?></span></p>
...
foreach ( $lists as $list ) {
	...
	esc_html_e( 'no lists selected', 'mymail' );
...
?>
		...
		<a class="create-new-list button" href="#"><?php esc_html_e( 'create new list', 'mymail' );?></a>
		<div class="create-new-list-wrap">
		<h4><?php esc_html_e( 'create a new list with all', 'mymail' );?></h4>
		<p>
...
foreach ( $options as $id => $option ) {?>
		...
		<p><a class="create-list button"><?php esc_html_e( 'create list', 'mymail' );?></a>
		</p>
		<p class="totals"><?php esc_html_e( 'Total receivers', 'mymail' );?>: <span id="mymail_total">-</span></p>
		</div>
		<?php
endif;
endif;?>
