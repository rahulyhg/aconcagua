...
<?php if ( $editable ): ?>
	...
	<p><label><input name="mymail_data[embed_images]" id="mymail_data_embed_images" value="1" type="checkbox" <?php echo ( isset( $this->post_data['embed_images'] ) ) ? ( ( $this->post_data['embed_images'] ) ? 'checked' : '' ) : ( mymail_option( 'embed_images' ) ? 'checked' : '' ) ?> <?php echo ( $editable ) ? 'disabled' : '' ?>> <?php esc_html_e( 'Embed Images', 'mymail' )?></label></p>
	<label><?php esc_html_e( 'Colors', 'mymail' );?></label> <a class="savecolorschema"><?php esc_html_e( 'save this schema', 'mymail' )?></a>
...
?>
	...
	<div class="clear"></div>
	<p>
	<label><?php esc_html_e( 'Colors Schemas', 'mymail' );?></label>
	<?php
$customcolors = get_option( 'mymail_colors' );
if ( isset( $customcolors[$this->get_template()] ) ):
?>
	<a class="colorschema-delete-all"><?php esc_html_e( 'delete all custom schemas', 'mymail' )?></a>
	<?php endif;?>
	</p>
	<ul class="colorschema" title="<?php esc_html_e( 'original', 'mymail' )?>">
	<?php
...
if ( strtolower( implode( '', $original_colors_temp ) ) != strtolower( implode( '', $current_colors ) ) ): ?>
	<ul class="colorschema" title="<?php esc_html_e( 'current', 'mymail' )?>">
	<?php
...
?>
	<hr>
	<label><?php esc_html_e( 'Background', 'mymail' )?></label><br>
	<?php
...
?>
			<ul data-base="<?php echo MYMAIL_URI ?>">
				<li><a title="<?php esc_html_e( 'none', 'mymail' )?>" data-file="" <?php if ( !$value ) {
	echo ' class="active"';
}
?>><?php esc_html_e( 'none', 'mymail' )?></a></li>
		<?php
...
?>
	...
	<p class="howto tiny"><?php esc_html_e( 'background images are not displayed on all clients!', 'mymail' )?></p>

<?php else: ?>

	<p><?php if ( $this->post_data['embed_images'] ) {?>&#10004;<?php } else {?>&#10005;<?php }?> <?php esc_html_e( 'Embedded Images', 'mymail' )?></p>
	<label><?php esc_html_e( 'Colors Schema', 'mymail' )?></label><br>
