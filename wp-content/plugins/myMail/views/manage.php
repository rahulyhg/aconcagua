...
<h2 class="nav-tab-wrapper">
	...
	<a class="nav-tab <?php echo ( 'import' == $currentpage ) ? 'nav-tab-active' : '' ?>" href="edit.php?post_type=newsletter&page=mymail_subscriber-manage&tab=import"><?php esc_html_e( 'Import', 'mymail' )?></a>
	<?php endif;?>

	<?php if ( current_user_can( 'mymail_export_subscribers' ) ): ?>
	<a class="nav-tab <?php echo ( 'export' == $currentpage ) ? 'nav-tab-active' : '' ?>" href="edit.php?post_type=newsletter&page=mymail_subscriber-manage&tab=export"><?php esc_html_e( 'Export', 'mymail' )?></a>
	<?php endif;?>

	<?php if ( current_user_can( 'mymail_bulk_delete_subscribers' ) ): ?>
	<a class="nav-tab <?php echo ( 'delete' == $currentpage ) ? 'nav-tab-active' : '' ?>" href="edit.php?post_type=newsletter&page=mymail_subscriber-manage&tab=delete"><?php esc_html_e( 'Delete', 'mymail' )?></a>
	<?php endif;?>
	...
	<div class="step1">
		<div class="step1-body">
			<div class="upload-method">
				<h2><?php esc_html_e( 'Upload', 'mymail' );?></h2>
				<p class="description"><?php esc_html_e( 'upload you subscribers as comma-separated list (CSV)', 'mymail' );?></p>
			...
			<div class="upload-method">
				<h2><?php esc_html_e( 'Paste', 'mymail' );?></h2>
				<p class="description"><?php esc_html_e( 'Copy and paste from your spreadsheet app', 'mymail' );?></p>
				<textarea id="paste-import" class="widefat" rows="13" placeholder="<?php esc_html_e( 'paste your list here', 'mymail' );?>">
...
<?php if ( current_user_can( 'mymail_import_wordpress_users' ) ): ?>
	...
	<div id="wordpress-users">
		<h2><?php esc_html_e( 'WordPress Users', 'mymail' );?></h2>
...
if ( !empty( $roles ) ): ?>
			<div id="wordpress-user-roles">
				<h4><?php esc_html_e( 'Import WordPress users with following roles', 'mymail' );?></h4>
				<p><label><input type="checkbox" class="wordpress-users-toggle" checked> <?php esc_html_e( 'toggle all', 'mymail' );?></label></p>
				<ul>
				<?php
$i = 0;
...
?>
				...
				<ul>
					<li><label><input type="checkbox" name="no_role" value="1" checked> <?php esc_html_e( 'users without a role', 'mymail' );?></label></li>
				</ul>
			</div>
...
?>
				<h4><?php esc_html_e( 'Use following meta values', 'mymail' );?></h4>
				<p><label><input type="checkbox" class="wordpress-users-toggle"> <?php esc_html_e( 'toggle all', 'mymail' );?></label></p>
				<ul>
				<?php
...
?>
			...
			<div class="clearfix clear">
				<input type="submit" class="button button-primary button-large" value="<?php esc_html_e( 'Next Step', 'mymail' );?> &#x2192;">
			</div>
		</form>
	</div>
...
echo $d?>">
			...
			<p>
				<label><?php esc_html_e( 'Output Format', 'mymail' )?>:
				<select name="outputformat">
					<option value="csv" selected><?php esc_html_e( 'CSV', 'mymail' );?></option>
					<option value="html" ><?php esc_html_e( 'HTML', 'mymail' );?></option>
				</select>
				</label>
			</p>
			<p>
				<label><?php esc_html_e( 'Separator for CSV output', 'mymail' )?>:
				<select name="separator">
					<option value=";" selected>;</option>
					<option value="," >,</option>
					<option value="|" >|</option>
					<option value="tab" ><?php esc_html_e( '[Tab]', 'mymail' );?></option>
				</select>
				</label>
			</p>
			<p>
				<label><?php esc_html_e( 'CharSet', 'mymail' )?>:
				<?php $charsets = array(
	'UTF-8' => 'Unicode 8',
	...
	'ISO-8859-16' => 'South-Eastern European',
			...
			<p>
				<label><?php esc_html_e( 'MySQL Server Performance', 'mymail' )?>:
				<select name="performance" class="performance">
					<option value="1000"><?php esc_html_e( 'low', 'mymail' );?></option>
					<option value="5000" selected><?php esc_html_e( 'normal', 'mymail' );?></option>
					<option value="10000"><?php esc_html_e( 'high', 'mymail' );?></option>
					<option value="20000"><?php esc_html_e( 'super high', 'mymail' );?></option>
					<option value="50000"><?php esc_html_e( 'super extreme high', 'mymail' );?></option>
				</select>
				</label>
			</p>
			<h3>
			<?php esc_html_e( 'Define order and included columns', 'mymail' );?>:<br>
			</h3>
			<?php
...
?>
			...
			<p>
				<input class="button button-large button-primary" type="submit" value="<?php esc_html_e( 'Download Subscribers', 'mymail' )?>" />
			</p>
			</form>
		</div>
	...
	<?php else: ?>
		...
		<p><?php esc_html_e( 'no subscriber found', 'mymail' );?></p>

	<?php endif;?>

<?php elseif ( 'delete' == $currentpage && current_user_can( 'mymail_bulk_delete_subscribers' ) ): ?>

			<h2 class="delete-status"><?php esc_html_e( 'Delete Subscribers', 'mymail' )?></h2>
			<?php
...
if ( !empty( $lists ) || $no_list ): ?>
			...
			<div class="step1">
				...
				<?php if ( !empty( $lists ) ): ?>
				<ul>
				<li><label><input type="checkbox" class="list-toggle"> <?php esc_html_e( 'toggle all', 'mymail' ); ?></label></li>
				<li>&nbsp;</li>
				<?php mymail( 'lists' )->print_it( null, false, 'lists', __( 'total', 'mymail' ) ); ?>
				</ul>
				<?php endif;?>
				...
				<p>
					<label><input type="checkbox" name="remove_lists" value="1"> <?php esc_html_e( 'remove selected lists', 'mymail' );?> </label>
				</p>
				<p>
					<label><input type="checkbox" name="remove_actions" value="1" checked> <?php esc_html_e( 'remove all actions from affected users', 'mymail' );?> </label>
				</p>
				<p>
					<input class="button button-large button-primary" type="submit" value="<?php esc_html_e( 'Delete Subscribers permanently', 'mymail' )?>" />
				</p>
				</form>
			</div>
	<?php else: ?>
...
<?php else: ?>
	...
	<h2><?php esc_html_e( 'You do not have sufficient permissions to access this page.', 'mymail' )?></h2>
