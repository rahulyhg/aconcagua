...
if ( $is_new ) {
	esc_html_e( 'Add new List', 'mymail' );
} else {
	...
	printf( __( 'Edit List %s', 'mymail' ), '<strong>' . $list->name . '</strong>' );
?>
<?php if ( current_user_can( 'mymail_add_subscribers' ) ): ?>
 <a href="edit.php?post_type=newsletter&page=mymail_lists&new" class="add-new-h2"><?php esc_html_e( 'Add New', 'mymail' );?></a>
<?php endif;?>
...
<span class="alignright">
	<?php if ( !$is_new && current_user_can( 'mymail_delete_lists' ) ): ?>
		<input type="submit" name="delete" class="button button-large" value="<?php esc_html_e( 'delete List', 'mymail' );?>" onclick="return confirm('<?php esc_attr_e( 'Do you really like to remove this list?', 'mymail' );?>');">
	<?php endif;?>
	<?php if ( !$is_new && current_user_can( 'mymail_delete_lists' ) && current_user_can( 'mymail_delete_subscribers' ) ): ?>
		<input type="submit" name="delete_subscribers" class="button button-large" value="<?php esc_html_e( 'delete List with Subscribers', 'mymail' );?>" onclick="return confirm('<?php esc_attr_e( 'Do you really like to remove this list with all subscribers?', 'mymail' );?>');">
	<?php endif;?>
...
<table class="form-table">
	<tr>
		<th scope="row"><h3><?php esc_html_e( 'Name', 'mymail' )?></h3></th>
		<td>
			<h3 class="detail">
				<ul class="click-to-edit">
					<li><?php echo esc_attr( $list->name ); ?>&nbsp;</li>
					<li><input id="name" class="widefat" type="text" name="mymail_data[name]" value="<?php echo esc_attr( $list->name ); ?>" placeholder="<?php esc_html_e( 'Name of the List', 'mymail' )?>"></li>
				</ul>
			</h3>
		</td>
	</tr>
	<tr>
		<th scope="row"><?php esc_html_e( 'Description', 'mymail' )?></th>
		<td>
			<div class="detail">
				<ul class="click-to-edit">
	...
	<tr>
		<th scope="row"><?php esc_html_e( 'Subscribers', 'mymail' )?></th>
		<td>
...
?>
		<div class="stats-wrap">
			<table id="stats">
				...
				</td><td><span class="verybold"></span> <?php esc_html_e( 'open rate', 'mymail' )?></td>
				<td width="60">
					<div id="stats_click" class="piechart" data-percent="<?php echo $clickrate ?>"><span>0</span>%</div>
				</td><td><span class="verybold"></span> <?php esc_html_e( 'click rate', 'mymail' )?></td>
				<td width="60">
					<div id="stats_unsub" class="piechart" data-percent="<?php echo $unsubscriberate ?>"><span>0</span>%</div>
				</td><td><span class="verybold"></span> <?php esc_html_e( 'unsubscribe rate', 'mymail' )?></td>
				<td width="60">
					<div id="stats_bounce" class="piechart" data-percent="<?php echo $bouncerate ?>"><span>0</span>%</div>
				</td><td><span class="verybold"></span> <?php esc_html_e( 'bounce rate', 'mymail' )?></td>
				</tr>
			</table>
		</div>
...
?>
	...
	case 1:
		...
		printf( __( 'Campaign %s has start sending', 'mymail' ), '<a href="' . admin_url( 'post.php?post=' . $activity->campaign_id . '&action=edit' ) . '">' . $activity->campaign_title . '</a>' );
		break;
	case 2:
		echo '<span class="mymail-icon icon-mm-open"></span></td><td>';
		printf( __( 'First open in Campaign %s', 'mymail' ), '<a href="' . admin_url( 'post.php?post=' . $activity->campaign_id . '&action=edit' ) . '">' . $activity->campaign_title . '</a>' );
		break;
	case 3:
		echo '<span class="mymail-icon icon-mm-click"></span></td><td>';
		printf( __( '%1$s in Campaign %2$s clicked', 'mymail' ), '<a href="' . $activity->link . '">' . __( 'Link', 'mymail' ) . '</a>', '<a href="' . admin_url( 'post.php?post=' . $activity->campaign_id . '&action=edit' ) . '">' . $activity->campaign_title . '</a>' );
		break;
	case 4:
