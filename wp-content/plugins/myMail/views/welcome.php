...
<?php if ( !mymail_option( 'purchasecode' ) ): ?>
	<div class="license">
		<?php if ( isset( $_POST['purchasecode'] ) && !empty( $_POST['purchasecode'] ) ):
			...
			$purchasecode = esc_attr( $_POST['purchasecode'] );

			$verified = UpdateCenterPlugin::verify(MYMAIL_SLUG, $purchasecode);

			if(is_wp_error($verified)) : ?>

			<h3>Sorry, the provided license code is not accepted!</h3>
			<p><?php echo mymail()->get_update_error($verified->get_error_code()) ?></p>
			<p><a class="button button-primary external" href="https://mymail.newsletter-plugin.com/buy/?utm_campaign=plugin&utm_medium=welcome+page"><strong><?php printf( __( 'Buy an additional license for %s.', 'mymail' ), ( mymail_is_local() ? __( 'your new site', 'mymail' ) : $_SERVER['HTTP_HOST'] )) ?></strong></a> or <a href="<?php echo admin_url( 'edit.php?post_type=newsletter&page=mymail_welcome' ); ?>">enter a different one</a></p>

			<?php else : ?>

			<?php mymail_update_option( 'purchasecode', $purchasecode ) ?>
		...
		<?php else: ?>
			...
			<form action="edit.php?post_type=newsletter&page=mymail_welcome" method="POST">
				<input type="text" class="widefat purchasecode" placeholder="XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX" name="purchasecode" required pattern="[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}" maxlength="36">
...
?>
		<div class="update-nag"><p><strong><?php esc_html_e( 'MyMail is available in your language.', 'mymail' )?></strong> <a class="button button-primary button-small" href="<?php echo esc_url( $url ) ?>"><?php esc_html_e( 'load it now', 'mymail' );?></a></p></div>
		<?php
endif;
?>
