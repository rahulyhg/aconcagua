...
if ( $is_new ) {
	esc_html_e( 'Add new Subscriber', 'mymail' );
} else {
	printf( __( 'Edit %s', 'mymail' ), '<strong>' . $nicename . '</strong>' );
...
<?php if ( current_user_can( 'mymail_add_subscribers' ) ): ?>
 <a href="edit.php?post_type=newsletter&page=mymail_subscribers&new" class="add-new-h2"><?php esc_html_e( 'Add New', 'mymail' );?></a>
<?php endif;?>
<?php if ( $subscriber->wp_id ): ?><a href="user-edit.php?user_id=<?php echo $subscriber->wp_id ?>" class="add-new-h2"><?php esc_html_e( 'goto WordPress User profile', 'mymail' );?></a><?php endif;?>
<?php }?>
	<span class="alignright">
		<?php if ( !$is_new && $subscriber->status == 0 ): ?>
			<input type="submit" name="confirmation" class="button button-large" value="<?php esc_html_e( 'resend confirmation', 'mymail' );?>" onclick="return confirm('<?php esc_attr_e( 'Do you really like to resend the confirmation?', 'mymail' );?>');">
		<?php endif;?>
		<?php if ( !$is_new && current_user_can( 'mymail_delete_subscribers' ) ): ?>
			<input type="submit" name="delete" class="button button-large" value="<?php esc_html_e( 'delete Subscriber', 'mymail' );?>" onclick="return confirm('<?php esc_attr_e( 'Do you really like to remove this subscriber?', 'mymail' );?>');">
		<?php endif;?>
		<input type="submit" name="save" class="button button-primary button-large" value="<?php esc_html_e( 'Save', 'mymail' );?>">
	</span>
</h2>
...
?>" title="<?php esc_html_e( 'Source', 'mymail' )?>: Gravatar.com" style="background-image:url(<?php echo $this->get_gravatar_uri( $subscriber->email, 400 ); ?>)"></div>
			<p class="info"><?php esc_html_e( 'Source', 'mymail' )?>: <a href="http://gravatar.com">Gravatar.com</a></p>
			<?php if ( !$is_new ): ?>

			<h4 title="<?php esc_html_e( 'The user rating is based on different factors like open rate, click rate and bounces', 'mymail' )?>"><?php esc_html_e( 'User Rating', 'mymail' );?>:<br />
			<?php
	...
	. str_repeat( '<span class="mymail-icon icon-mm-star-empty"></span>', $empty ) ?>
		...
		<td class="user-info">
			<h3 class="detail">
				...
				<code title="<?php printf( __( 'use %1$s as placeholder tag to replace it with %2$s', 'mymail' ), '{emailaddress}', '&quot;' . $subscriber->email . '&quot;' ) ?>">{emailaddress}</code>
			</h3>
			<div class="detail">
				<label for="mymail_firstname" class="label-type-name"><?php esc_html_e( 'Name', 'mymail' );?>:</label>
				...
				<code title="<?php printf( __( 'use %1$s as placeholder tag to replace it with %2$s', 'mymail' ), '{fullname}', '&quot;' . $subscriber->fullname . '&quot;' ) ?>">{fullname}</code>
				<code title="<?php printf( __( 'use %1$s as placeholder tag to replace it with %2$s', 'mymail' ), '{lastname}', '&quot;' . $subscriber->lastname . '&quot;' ) ?>">{lastname}</code>
				<code title="<?php printf( __( 'use %1$s as placeholder tag to replace it with %2$s', 'mymail' ), '{firstname}', '&quot;' . $subscriber->firstname . '&quot;' ) ?>">{firstname}</code>
			</div>
			<div class="detail">
				<label for="mymail_status"><?php esc_html_e( 'Status', 'mymail' );?>:</label>
...
?>
						...
						<span class="description info"><?php esc_html_e( 'choosing "pending" as status will force a confirmation message to the subscriber', 'mymail' );?></span>
						</div>
					</li>
				</ul>
			</div>
			<?php if ( !$is_new ): ?>
			<div class="info">
				<strong><?php esc_html_e( 'subscribed at', 'mymail' );?>:</strong> <?php echo $subscriber->signup
	...
	: __( 'unknown', 'mymail' ) ?>
				...
				<a class="show-more-info alignright"><?php esc_html_e( 'more', 'mymail' );?></a>
				<ul class="more-info">
					<li><strong><?php esc_html_e( 'confirmed at', 'mymail' );?>:</strong> <?php echo $subscriber->confirm
	...
	: __( 'unknown', 'mymail' ) ?>
					...
					<li><strong><?php esc_html_e( 'latest known IP', 'mymail' );?>:</strong> <?php echo $meta->ip
	? $meta->ip
	: __( 'unknown', 'mymail' ) ?>
					</li>
				</ul>
			...
			<div class="info">
				<strong><?php esc_html_e( 'latest updated', 'mymail' );?>:</strong> <?php echo $subscriber->updated
...
?>
				<div class="detail">
					<label for="mymail_data_<?php echo $field ?>" class="label-type-<?php echo $data['type'] ?>"><?php echo $data['name'] ?>:</label>
						<code title="<?php printf( __( 'use %1$s as placeholder tag to replace it with %2$s', 'mymail' ), '{' . $field . '}', '&quot;' . $subscriber->{$field} . '&quot;' ) ?>">{<?php echo $field ?>}</code>
...
?>
			...
			<div class="detail v-top">
				<label><?php esc_html_e( 'Lists', 'mymail' );?>:</label>
			...
			<?php if ( !$is_new ): ?>
				<?php if ( $meta->coords ): $geo = explode( '|', $meta->geo );?>
					...
					<?php $mapurl = add_query_arg( array(
							...
							'scale' => 2,
							'language' => get_locale(),
							'key' => mymail_option( 'google_api_key' ),
				...
				<div class="map">
					<?php $mapurl = add_query_arg( array(
							'center' => mymail( 'geo' )->code2Country( $geo[0] ),
							'zoom' => 3,
							'size' => '300x250',
							'visual_refresh' => true,
							'scale' => 2,
...
?>
		<div class="stats-wrap">
			<table id="stats">
				...
				<td><span class="verybold"></span> <?php esc_html_e( 'open rate', 'mymail' )?></td>
				<td width="60">
				<div id="stats_click" class="piechart" data-percent="<?php echo $clickrate * 100 ?>"><span>0</span>%</div>
				</td>
				<td><span class="verybold"></span> <?php esc_html_e( 'click rate', 'mymail' )?></td>
				<td width="60">
				<div id="stats_click" class="piechart" data-percent="<?php echo $aclickrate * 100 ?>"><span>0</span>%</div>
				</td>
				<td><span class="verybold"></span> <?php esc_html_e( 'adjusted click rate', 'mymail' )?></td>
				</tr>
			</table>
		</div>
...
?>
			...
			<h3><?php esc_html_e( 'Most popular client', 'mymail' )?>: <span class="mymail-icon client-<?php echo $mostpopular['type'] ?>"></span><?php echo $mostpopular['name'] . ' <span class="count">(' . round( $mostpopular['percentage'] * 100, 2 ) . '%)</span> ' ?></h3>

		<?php if ( !empty( $clients ) ): ?>
			<p><?php esc_html_e( 'Other used clients', 'mymail' )?>:
			<?php
...
?>
			<h3><?php esc_html_e( 'Activity', 'mymail' );?></h3>
				<p>
				<?php if ( $open_time ): ?>
				<?php printf( __( '%1$s needs about %2$s to open a campaign', 'mymail' ), ( $subscriber->fullname ? $subscriber->fullname : __( 'User', 'mymail' ) ), '<strong>' . human_time_diff( $now + $open_time ) . '</strong>' ); ?> <?php if ( $click_time ) {
	printf( __( 'and %1$s to click a link', 'mymail' ), '<strong>' . human_time_diff( $now + $click_time ) . '</strong>' );
}
?>
				<?php else:
	esc_html_e( 'User has never opened a campaign', 'mymail' );
endif;?>
			</p>
			<table class="wp-list-table widefat activities">
				<thead>
					<tr><th><?php esc_html_e( 'Date', 'mymail' );?></th><th></th><th><?php esc_html_e( 'Action', 'mymail' );?></th><th><?php esc_html_e( 'Campaign', 'mymail' );?></th><th></th></tr>
				</thead>
				<tbody>
...
?>
	...
	case 1:
		...
		printf( __( 'Campaign %s has been sent', 'mymail' ), '<a href="' . admin_url( 'post.php?post=' . $activity->campaign_id . '&action=edit' ) . '">' . $activity->campaign_title . '</a>' );
		break;
	case 2:
		echo '<span class="mymail-icon icon-mm-open"></span></td><td>';
		printf( __( 'opened Campaign %s', 'mymail' ), '<a href="' . admin_url( 'post.php?post=' . $activity->campaign_id . '&action=edit' ) . '">' . $activity->campaign_title . '</a>' );
		break;
	case 3:
		echo '<span class="mymail-icon icon-mm-click"></span></td><td>';
		printf( __( 'clicked %1$s in Campaign %2$s', 'mymail' ), '<a href="' . $activity->link . '">' . __( 'a link', 'mymail' ) . '</a>', '<a href="' . admin_url( 'post.php?post=' . $activity->campaign_id . '&action=edit' ) . '">' . $activity->campaign_title . '</a>' );
		break;
	case 4:
	...
	case 5:
		...
		printf( __( 'Soft bounce (%d tries)', 'mymail' ), $activity->count );
	...
	?></td>
						...
						<?php if ( $activity->campaign_status == 'trash' ): ?>
							<?php esc_html_e( 'campaign deleted', 'mymail' );?>
						<?php elseif ( $activity->type == 1 && current_user_can( 'publish_newsletters' ) ): ?>
							<a href="<?php echo add_query_arg( array( 'resendcampaign' => 1, '_wpnonce' => wp_create_nonce( 'mymail-resend-campaign' ), 'campaign_id' => $activity->campaign_id ) ); ?>" class="button button-small" onclick="return confirm('<?php printf( esc_attr__( 'Do you really like to resend campaign %1$s to %2$s?', 'mymail' ), "\\n\'" . $activity->campaign_title . "\'", "\'" . $nicename . "\'" ); ?>');">
							<?php esc_html_e( 'resend this campaign', 'mymail' );?>
							</a>
...
?>
		<p class="description"><?php esc_html_e( 'no activity yet', 'mymail' );?></p>
		<?php
endif;
?>
	</div>
