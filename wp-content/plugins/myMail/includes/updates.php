...
if ( $old_version ) {
	...
	case '2.1.25':
		...
		if ( isset( $mymail_options['smtp_auth'] ) ) $mymail_options['smtp_auth'] = 'LOGIN';
		if ( $mymail_options['php_mailer'] == '5.2.7' ) $mymail_options['php_mailer'] = false;

	case '2.1.26':
	case '2.1.27':

		if ( isset( $mymail_options['dkim'] ) && empty( $mymail_options['dkim_private_hash'] ) ) {
			$mymail_options['dkim_private_hash'] = md5( $mymail_options['dkim_private_key'] );
		}
	...
	case '2.1.30':
		...
		if ( isset( $mymail_options['php_mailer'] ) && $mymail_options['php_mailer'] ) {
			mymail_notice( '<strong>'.sprintf( 'PHPMailer has been updated to 5.2.21. <a href="%s">read more</a>', 'https://github.com/PHPMailer/PHPMailer/releases/tag/v5.2.20' ).'</strong>', '', false, 'phpmailer' );
			$mymail_options['php_mailer'] = 'latest';
		}
